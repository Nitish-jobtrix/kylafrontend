...
const Home = () => {
    ...
    const { keyword, location,companyName } = useParams();

    const [page, setPage] = useState(1);
    const [cat, setCat] = React.useState('');

    useEffect(() => {
        dispatch(jobLoadAction(page, keyword, cat, location,companyName));
                                    ...
                                    <MenuList>
                                        {
                                            setUniqueLocation && setUniqueLocation.map((location, i) => (
                                                <MenuItem key={i}>
                                                    ...
                                                    <Link style={{ color: palette.secondary.main }} to={`/jobs/${companyName}/search/location/${location}`}>{location}</Link>
                                                </MenuItem>
